...
if win32com.client.gencache.is_readonly:
    ...
    # NB You must ensure that the python...\win32com.client.gen_py dir
    # does not exist to allow creation of the cache in %temp%
...
geStatus = [None, "confirmed", None, "confirmed",
            None, "cancelled", None, "cancelled"]
        ...
        if value:
            if name == 'BYDAY':
                parts.append(name+'='+','.join(
                    str(rrule.weekdays[v]) for v in value))
            else:
                parts.append(name+'='+','.join(
...
class Outlook(object):
    ...
    def createCOM(self):
        success = False
        try:
            self.outlookCOM = win32com.client.gencache.EnsureDispatch(
                "Outlook.Application")
            success = True
    ...
    def isAlive(self):
        alive = False
        if self.outlookCOM is not None:
            try:
                # Try something
    ...
    def getNextRecDate(self, appt):
        ...
        now = dt.datetime.now()
        eDate = dt.datetime(now.year, now.month, now.day,
                            appt.Start.hour, appt.Start.minute)
        ...
        if rPattrn.RecurrenceType == msc.olRecursDaily:
            ...
            # print "gNRD: week %s, weekN %s, pattern: %d" %(repr(week),
            #    repr(weekN), rPattrn.DayOfWeekMask)
    ...
    def getRRULE(self, event):
        ...
        RRULE = rrule.rrule(freq, count=count, interval=interval,
                            until=until, byweekday=byweekday)
    ...
    def createEvent(self, appt):
        ...
        if appt.IsRecurring:
            ...
            #    dEnd = dEnd.replace(year=rDate.year, month=rDate.month,
            #                        day=rDate.day,hour=now.hour+2)
